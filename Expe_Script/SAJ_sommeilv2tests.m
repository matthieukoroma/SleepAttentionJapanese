...
if mycomp==0
    path_Expe_Folder='C:\Data\SleepAttentionJapanese\Expe_Folder\';
else
    path_Expe_Folder='/home/lscp00/Codes/Projects/attention japanese/SleepAttentionJapanese/Expe_Folder/';
end
cd(path_Expe_Folder)
...
if mycomp==0
    load([path_Expe_Folder,'\Expe_Material\TTLson']);
else
    load([path_Expe_Folder,'Expe_Material/TTLson']);
end
%%
...
while running&&exitK
    ...
    if mycomp==0
        ...
        load([path_Expe_Folder '/Expe_Material', '/testStims/', SessionStructureExp{currtrial}.FileName]);
    end
...
%     snrWN=0.4;
    ...
    elseif currtrial > nTotWake && currtrial <= nTotWake+nN2+1
        memorytest = 0;
        listnames.SleepN2.active = currtrial;
        set(h25,'String','N2 Phase');
    elseif currtrial >= nTotWake+nN2 && currtrial <= nTotWake+nTotSleep+1
        memorytest = 0;
        listnames.SleepN3.active = currtrial;
        set(h25,'String','N3 Phase');
    end
    if ismember(currtrial,TestTrialsAwake) || currtrial > nTotWake+nTotSleep+1
        memorytest = 1;
    else memorytest = 0;
    end
    ...
    %send to Netstation informations about the event
    if AcquisitionFlag==1
    ...
    if memorytest==1
        ...
        while waitingContinue
            ...
            if down==1 && sum(kCode)==1
                ...
                waitingContinue = 0;
                if AcquisitionFlag==1
                    [status, error] = NetStationPC_Debugged('Event',keyboardKey,timingKey,0.001,'Subj',str2num(Subject.ID),'nTri',countingTrial);
                end
            end
        end
...
% %         ResponseType{countingTrial,1} = StimCarac.TrialType;%source de l'erreur
    ...
    if training
        ...
        while waitingContinue
            ...
            if down==1 && sum(kCode)==1 && find(kCode)==keyboardpress
                waitingContinue = 0;
                if AcquisitionFlag==1
    ...
    if currtrial == nTotWake && nfalling>0
        ...
        listnames.falling.active =currtrial;
    elseif currtrial == nTotWake+nN2 && nN2>0 
        ...
        listnames.SleepN2.active =currtrial;
    elseif currtrial == nTotWake+nN2+nN3  && nN3>0

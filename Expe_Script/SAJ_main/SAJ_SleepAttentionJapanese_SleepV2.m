...
nTotTrials = nEEGTraining+nwake+nbehavTraining+4+nfalling+nN3+nN2;

endTrialJitter = fixInterval+jitterInterval*floor(rand(nTotTrials,1)*1000)/1000;
...
%     try
        ...
        elseif currtrial==nEEGTraining+nbehavTraining*2+1
            ...
            uiwait(nbtestkeyh);
        elseif ismember(currtrial,[Memory_Test_Transition.trial]-1)
            TTname = find([Memory_Test_Transition.trial]-1==currtrial);
            nbtestkeyh = msgbox(sprintf('Going to the ', Memory_Test_Transition(TTname).name, ' phase of Memory Test ?\n'),'Warning : Memory','warn');
            uiwait(nbtestkeyh);
...
%                 figure(ctrlPanel);
                ...
                if ~handleStatus.Active
                    if ~audioStopped
                        ...
                        if AcquisitionFlag==1
                            [status, error] = NetStationPC_Debugged('Event','endT',stopTimes(currevent,1),0.001,'Subj',str2num(Subject.ID),'Trai',StimCarac.TrialType,'nTri',playedTrial{currevent},...
...
%         figure(ctrlPanel);
        ...
        if rep==1 || ...
            ...
            pausing = GetSecs;
            if AcquisitionFlag==1
                [status, error] = NetStationPC_Debugged('Event','Paus',pausing,0.001,'Subj',str2num(Subject.ID),'nTri',playedTrial{currevent,1});
            end
            figure(ctrlPanel);
            uiwait(ctrlPanel);
            ContinueTime = GetSecs;
            if AcquisitionFlag==1
                [status, error] = NetStationPC_Debugged('Event','Cont',ContinueTime,0.001,'Subj',str2num(Subject.ID),'nTri',playedTrial{currevent,1});
            end
            set(h10,'String','');
        elseif currtrial~=1
            while GetSecs-stopTimes(currevent,1)<endTrialJitter(currtrial-1)
            end
        end
        if mycomp==0
        ...
        if training
            ...
            while waitingContinue
                ...
                if down==1 && sum(kCode)==1 %&& sum(ismember(find(kCode),AnswerKeys))>1
                    waitingContinue = 0;
                    if AcquisitionFlag==1
                        [status, error] = NetStationPC_Debugged('Event','spcP',secs,0.001,'Subj',str2num(Subject.ID),'nTri',playedTrial{currevent,1});
                    end
                end
            end
            set(h10,'String','');
        ...
        if rep==0
            ...
            if exitK==1
                playedTrial{currevent,1} = currtrial;
            end
        elseif rep==1
            if mycomp==0
            ...
            if exitK==1
                playedTrial{currevent,1} = currtrial-1;
            end
        end
...
%             sound(son(1:2,:)',fs)
            ...
            if AcquisitionFlag==1
                [status, error] = NetStationPC_Debugged('Event','newT',startTimes(currevent,1),0.001,'Subj',str2num(Subject.ID),'Trai',training,'nTri',playedTrial{currevent,1},...
                    'aCod',StimCarac.aCode,'aVoi',StimCarac.aVoice,'aSid',StimCarac.aSide,'aOrd',StimCarac.aOrder,...
                    'iCod',StimCarac.iCode);
            end
        end
        fprintf('New event: newT %g, Subj %s nTria %g \n\t\attended %d\n\t\ignored %d\n',num2str(startTimes(currevent)),Subject.ID,playedTrial{currevent,1},...
            num2str(StimCarac.aCode),...%StimCarac.afTag,StimCarac.aAmpTag,...dd
            num2str(StimCarac.iCode))%StimCarac.ifTag,StimCarac.iAmpTag)
        
        trialType{currevent,1} = StimCarac.TrialType;
        trialaCode{currevent,1} = StimCarac.aCode;
        trialaVoice{currevent,1} = StimCarac.aVoice;
        trialaSide{currevent,1} = StimCarac.aSide;
        trialaCategories{currevent,1} = StimCarac.aCategories;
        trialaName{currevent,1} = StimCarac.aName;
        trialaFreq{currevent,1} = StimCarac.aFreq;
        trialaPresented{currevent,1} = StimCarac.aPresented;
        trialjCode{currevent,1} = StimCarac.jCode;
        trialaOrder{currevent,1} = StimCarac.aOrder;
        
        trialiCode{currevent,1} = StimCarac.iCode;
        trialiVoice{currevent,1} = StimCarac.iVoice;
        trialiSide{currevent,1} = StimCarac.iSide;
        trialiCategories{currevent,1} = StimCarac.iCategories;
        trialiName{currevent,1} = StimCarac.iName;
        trialiFreq{currevent,1} = StimCarac.iFreq;
        trialiPresented{currevent,1} = StimCarac.iPresented;
        trialiOrder{currevent,1} = StimCarac.iOrder;
            ...
            while looping
                ...
                if ~handleStatus.Active
                    if ~audioStopped
                        stopTimes(currevent,1) = GetSecs();
                        if AcquisitionFlag==1
                            [status, error] = NetStationPC_Debugged('Event','endT',stopTimes(currevent,1),0.001,'Subj',str2num(Subject.ID),'Trai',training,'nTri',playedTrial{currevent,1},...
                                'aCod',StimCarac.aCode,'aVoi',StimCarac.aVoice,'aSid',StimCarac.aSide,'aOrd',StimCarac.aOrder,...%'aTag',StimCarac.afTag,'aAmp',StimCarac.aAmpTag*100,...
        ...
        if ismember(currtrial,TestTrialsAwake) || currtrial>nTotWake+nTotSleep
            ...
            while waitingContinue
                ...
                if down==1 && sum(kCode)==1
                    waitingContinue = 0;
                    if AcquisitionFlag==1
                        [status, error] = NetStationPC_Debugged('Event','spcP',timingKey,0.001,'Subj',str2num(Subject.ID),'nTri',playedTrial{currevent,1});
                    end
                end
                if down==1 && sum(kCode)==1 && ismember(find(kCode),AnswerKeys)
            ...
            ResponseType{currevent,1} = keyboardKey;
            ResponseRT{currevent,1} = timingKey;%temps réponse - timing du son
            Responseevent{currevent,1} = currevent;
            ResponseType{currevent,1} = StimCarac.TrialType;
            ResponseoCode{currevent,1} = StimCarac.aCode;
            ResponsenCode{currevent,1} = StimCarac.iCode;
            ResponsejCode{currevent,1} = StimCarac.jCode;
            ResponseVoice{currevent,1} = StimCarac.aVoice;
            ResponseoOrder{currevent,1} = StimCarac.aOrder;
            ResponseiOrder{currevent,1} = StimCarac.iOrder;
            ResponseaCategories{currevent,1} = StimCarac.aCategories;
            ResponseaFreq{currevent,1} = StimCarac.aFreq;
            ResponseiCategories{currevent,1} = StimCarac.iCategories;
            ResponseaPresented{currevent,1} = StimCarac.aPresented;
            ResponseiFreq{currevent,1} = StimCarac.iFreq;
            ResponseiPresented{currevent,1} = StimCarac.iPresented;
...
save(saveName,'Subject','Timings','TrialsCaracs','ResponseCaracs','SessionStructureExp')

...
if stems>1;
    test_items_position = randi(stems+1,[1,nwakeTest]);
    ...
    vectorwakeTestitems(:,ind)=[vectorwakeTestitems(1,ind)-1;repmat(3,1,length(ind))];
    vectorwakeTestTrials = [[test_position];repmat(4,1,length(test_items_position))];
elseif stems==1;
    test_items_position = randi(2,[1,nwakeTest]);
    test_items_position(test_items_position==2)=3;
    vectorwakeTestitems = [test_position;test_items_position];
    vectorwakeTestTrials = [[test_position];repmat(4,1,length(test_items_position))];
elseif stems==0
    vectorwakeTestitems=[1:nwakeTest;repmat(1,1,nwakeTest)];
    vectorwakeTestTrials=[1:nwakeTest;repmat(4,1,nwakeTest)];
end
vectorTest = [vectorwake,vectorwakeTestitems,vectorwakeTestTrials];
sortvectorTest=sortrows(vectorTest',[1,2]);
%rearrange vectors

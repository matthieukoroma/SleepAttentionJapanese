...
%     TestTrials = [nEEGTraining+2:2:nbehavTraining,nEEGTraining+2*nbehavTraining+vectorTestind'];
    
    %arrange SessionStructure
end
nTotTrials = length(SessionStructureExp);
...
%     end
    ...
    if ismember(currtrial,[Memory_Test_Transition.trial])
        ...
        nbtestkeyh = msgbox(sprintf(['Going to the Phase ?\n']),'Warning : Memory','warn');
        uiwait(nbtestkeyh);
    end

    ...
    for stim=1:length(pairs)
        ...
        for voice=1:length(voices)
            for ind = 1:length(ind_p)
                name_write = [voices(voice),num2str(Codes(ind_p(ind)))];%
                name_write = [name_write{:}];
                cd('/home/lscp00/Matthieu/SleepAttentionJapanese/Expe_Material/stimsTrials/')
                clear Stim
                load([name_write,'.mat'])
                stim_m = Stim.Mat(:,1);
                %              stimF =[];
                ...
                %             length(s)
                %             Stim.Mat = s;
                nam = ['/home/lscp00/Matthieu/SleepAttentionJapanese/Expe_Material/stimsTrialsVsolamin', tech{:}]
                 cd(nam{:})

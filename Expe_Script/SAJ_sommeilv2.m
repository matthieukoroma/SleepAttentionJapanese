...
if mycomp==0
    ...
    cd('/home/lscp00/Matthieu/SleepAttentionJapanese/')
end
...
if mycomp==0
    ...
    load('/home/lscp00/Matthieu/SleepAttentionJapanese/Expe_Material/TTLson');
end
%%
...
countingPresentation = zeros(1,length(Codes));
...
% if nN2>0
%     h23 = uicontrol('Position', [120 40 100 20], 'String', 'N2 Phase', 'Callback', sprintf('currtrial=listnames.SleepN2.active;Changelist = 1;\nset(h25,''String'',''N2 Phase'');\ndrawnow;\nset(h25,''Enable'',''on'')\n'));
% end;if nN3>0;h26 = uicontrol('Position', [120 20 100 20], 'String', 'N3 Phase', 'Callback', sprintf('currtrial=listnames.SleepN3.active;Changelist = 1;\nset(h25,''String'',''N3 Phase'');\ndrawnow;\nset(h25,''Enable'',''on'')\n'));
% end;if nfalling>0; h24 = uicontrol('Position', [120 60 100 20], 'String', 'Falling asleep Phase', 'Callback', sprintf('currtrial=listnames.falling.active;Changelist = 1;\nset(h25,''String'',''Falling asleep Phase'');\ndrawnow;\nset(h25,''Enable'',''on'')\n'));
...
while running&&exitK
    ...
    if mycomp==0
        ...
        load(['/home/lscp00/Matthieu/SleepAttentionJapanese/Expe_Material', '/testStimsafLKFS/', SessionStructureExp{currtrial}.FileName]);
    end
    son = Stim.Mat;
    ...
    currtrial
    if currtrial > nTotWake-nfalling+1 && currtrial <= nTotWake
    ...
    elseif currtrial==nEEGTraining+nbehavTraining*2+1
        ...
        uiwait(nbtestkeyh);
    elseif currtrial == nTotWake-nfalling+1 && falling==0
    ...
    if ismember(currtrial,[Memory_Test_Transition.trial])
        TTname = find([Memory_Test_Transition.trial]==currtrial);
        set(h25,'String','Memory_Test Phase');
        nbtestkeyh = msgbox(sprintf(['Going to the ' Memory_Test_Transition(TTname).name 'Phase ?\n']),'Warning : Memory','warn');
        uiwait(nbtestkeyh);
    end
    ...
    if memorytest
        nextStimCarac = struct('FileName',SessionStructureExp{currtrial}.FileName,...
            'TrialType',SessionStructureExp{currtrial}.TrialType,'Condition',SessionStructureExp{currtrial}.TrialType,...
            ...
            'aPresented',countingPresentation(SessionStructureExp{currtrial}.old.Code),...
            ...
            'iSide',SessionStructureExp{currtrial}.new.Side,'jCode',SessionStructureExp{currtrial}.jap.Code,'iPresented',countingPresentation(SessionStructureExp{currtrial}.new.Code));
    else
        SessionStructureExp{currtrial}.attended.Presented = SessionStructureExp{currtrial}.attended.Presented+1;
        SessionStructureExp{currtrial}.ignored.Presented = SessionStructureExp{currtrial}.ignored.Presented+1;
        countingPresentation(SessionStructureExp{currtrial}.attended.Code)=countingPresentation(SessionStructureExp{currtrial}.attended.Code)+1;
        countingPresentation(SessionStructureExp{currtrial}.ignored.Code)=countingPresentation(SessionStructureExp{currtrial}.ignored.Code)+1;
        nextStimCarac = struct('FileName',SessionStructureExp{currtrial}.FileName,...
            'TrialType',SessionStructureExp{currtrial}.TrialType,'Condition',SessionStructureExp{currtrial}.TrialType,...
    ...
    set(h9,'String',sprintf('Trial %d',currtrial));
    if mycomp==0
        handleStatus = PsychPortAudio('GetStatus',pahandle);
    end
    ...
    trialCondition{countingTrial,1} = StimCarac.Condition;
...
%     figure(ctrlPanel);
    ...
    fprintf('>>>> Wait - Type something if the Subject is changing state or to QUIT\n'); fprintf(' \n')
    if countNoResponse<5
        if countNoResponse==0
            fprintf('Subject is not responding since %g trials\n',countNoResponse)
            LastResponse=GetSecs;
        else
            fprintf('Subject is not responding since %g trials and %g minutes\n',countNoResponse,(GetSecs-LastResponse)/60)
        end
    else
        if BlockID==1
            fprintf('Subject is not responding since %g trials and %g minutes -> CHECK if sleeping\n',countNoResponse,(GetSecs-LastResponse)/60)
        elseif BlockID==2
            fprintf('Subject is not responding since %g trials and %g minutes -> CHECK if STILL sleeping\n',countNoResponse,(GetSecs-LastResponse)/60)
        end
    end
    
    fprintf(' \n')
    if totTrial>1
        fprintf('Subject has a failure rate of %g (%g / %g)\n',(sum(isnan(BehavioralResults(:,12))==0)-nansum(BehavioralResults(:,12)))/sum(isnan(BehavioralResults(:,12))==0),(sum(isnan(BehavioralResults(:,12))==0)-nansum(BehavioralResults(:,12))),sum(isnan(BehavioralResults(:,12))==0)); fprintf(' \n')
    end
    fprintf('We are on block %s \n' , BlockNames{BlockID}); fprintf(' \n')
    ...
    if memorytest==1
        ...
        while waitingContinue
            ...
            if down==1 && sum(kCode)==1
                ...
                waitingContinue = 0;
                if AcquisitionFlag==1
                    [status, error] = NetStationPC_Debugged('Event','Resp',timingKey,0.001,'Subj',str2num(Subject.ID),'nTri',countingTrial);
                end
            end
        end
...
% %         ResponseType{countingTrial,1} = StimCarac.TrialType;%source de l'erreur
        ResponseCondition{countingTrial,1} = StimCarac.Condition;
        ...
        ResponseCondition{countingTrial,1} = nan;
    ...
    elseif currtrial == nTotWake+nN2+nN3
        ...
        listnames.SleepN3.active =currtrial;
    elseif currtrial == nTotWake+nN2+nN3+1 && memorytest==0;
        %memory phase
    else
...
%         uiwait(ctrlPanel);
        ...
        %         if currtrial~=startingTrial
        while GetSecs-countTime<endTrialJitter(countingTrial)
        end
        %         end
    end
...
TrialsCaracs = struct(  'Trials',playedTrial,...
    'TrialType',trialType,'Condition',trialCondition,...
...
ResponsesCaracs = struct('Type',ResponseType,'RT',ResponseRT,...
    'event',Responseevent,'TrialType',trialType,'Condition',trialCondition,...
